...
describe('GiftsEffects', () => {
  ...
  it('should return a LOAD_SPECIALS_COMPLETE action with expected gifts as payload', () => {
    runner.queue(new GiftsActions.LoadSpecialsAction());

    giftsEffects.load$.subscribe(result => {
      expect(result).toEqual(new GiftsActions.LoadSpecialsCompleteAction(giftsPayload));
    });
  });
});
